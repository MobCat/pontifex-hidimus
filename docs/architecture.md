# Phidimus architecture (Engine v4.2.0)
...
### How lives in Go, what lives in the module

One line splits the whole project: **the engine is *how* to translate; a module is
*what* to translate.**

- **How — in Go, in the engine.** The mechanics of a translation: the USB device
  state machine, the raw-gadget / USB-IP transports, the crypto and byte-mangling of
  an authentication scheme. Hard, reusable machinery, written and tested once. XSM3
  (the wired Xbox 360's controller authentication) is a *how* — the algorithm that
  turns a console challenge into the correct response lives in Go.

- **What — in a QC module, as data.** The identity and the secrets: descriptors,
  report layouts, signal names, and any **keys or per-device data** an algorithm
  needs. XSM3's *keys* are a *what* — they live in the module and are handed to the Go
  algorithm at run time. The engine never bakes them in.

Why the line is drawn exactly there:

- **Keys and data stay independent of our code.** They can be distributed, updated, or
  **licensed separately** without shipping a new engine — and material we can't or
  won't bundle stays out of the binary entirely.
- **No monolith.** Phidimus is not "one blob that knows every controller." Anyone can
  author a module for the device *they* need against the stable Go primitives and use
  it immediately — they never wait on us to add their device to a central list.
- **As little device-specific code in Go as possible.** A new controller should be a
  new *module*, not a new engine branch. Go grows only when a genuinely new
  *mechanism* appears (a new transport, a new auth method) — a reusable *how* many
  modules then draw on.

The test for "where does this go?" is: *is it a reusable mechanism, or is it identity
and data for one device?* Mechanisms are Go. Identity, layouts, and keys are the module.
...
A v4.0+ output module *is* a USB device. See `QC_USB_API_v4.md`.
